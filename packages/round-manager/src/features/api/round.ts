...
export async function getRoundById(
  ...
  try {
    ...
    const [roundMetadata, applicationMetadata] = await Promise.all([
      fetchFromIPFS(res[0].roundMetaPtr.pointer),
      fetchFromIPFS(res[0].applicationMetaPtr.pointer),
    ]);

    const operatorWallets = res[0].roles[0].accounts.map(
    ...
    return {
      ...
      roundMetadata,
      applicationMetadata,
      applicationsStartTime: new Date(res[0].applicationsStartTime * 1000),
      applicationsEndTime: new Date(res[0].applicationsEndTime * 1000),
      roundStartTime: new Date(res[0].roundStartTime * 1000),
      roundEndTime: new Date(res[0].roundEndTime * 1000),
      token: res[0].token,
      votingStrategy: res[0].votingStrategy,
      ownedBy: res[0].program.id,
